        ...
        {{-- <div class="mt-20 mb-16">
            @foreach ($messages as $message)
                @if ($message['sender'] != auth()->user()->name)
                    <div class="clearfix">
                        <div class="bg-gray-300 w-3/4 mx-4 my-2 p-2 rounded-lg">this is a basic mobile chat layout,
                            build with tailwind css</div>
                    </div>
                @else
                    <div class="clearfix">
                        <div class="bg-green-300 float-right w-3/4 mx-4 my-2 p-2 rounded-lg clearfix">check my twitter
                            to see when it will be released.</div>
                    </div>
                @endif
            @endforeach
        </div>
        ...
        @foreach ($messages as $message)
            @if ($message['sender'] != auth()->user()->name)
                <div class="flex justify-start">
                    <div class="bg-gray-300 max-w-xs mx-4 my-2 p-4 rounded-lg break-words">
                        <b>{{ $message['sender'] }} : </b> {{ $message['message'] }}
                    </div>
                </div>
            @else
                <div class="flex justify-end">
                    <div class="bg-green-300 max-w-xs mx-4 my-2 p-4 rounded-lg break-words">
